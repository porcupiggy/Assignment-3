...
$(document).ready(function () {
    $('body').prepend(`
    <div id="loadingStatus" class="text-info text-center my-2" style="display:none;">
        Loading joke...
    </div>
    <div id="errorStatus" class="text-danger text-center my-2" style="display:none;">
    ...
    $('#jokesContainer').on('click', '.btn-fav', function () {
...
function renderJokes() {
    ...
    $.each(jokes, function (index, joke) {
        const jokeCard = $(`
            <div class="card quote-card mb-3" data-id="${joke.id}">
                <div class="card-body">
                    <p class="quote-text">"${joke.setup}"</p>
                    <p class="quote-author">— ${joke.punchline}</p>
                    <button class="btn btn-warning btn-sm btn-fav" data-id="${joke.id}">
    ...
    $('.btn-dismiss').on('click','btn-dismiss', function () {
        const jokeId = $(this).data('id');
        dismissJoke(jokeId);
    });
}
